...
The codebase was built during the net8.0→net9.0 transition when Microsoft.OpenApi shipped breaking API changes. To support both, we multi-targeted (`net8.0;net9.0` for core, `net9.0;net10.0` for OpenApi producer) with `#if NET9_0_OR_GREATER` and `#if NET10_0_OR_GREATER` preprocessor directives throughout. This resulted in duplicated code paths for:

1. **Spec loading** — `OpenApiStreamReader` (v1) vs `OpenApiDocument.Parse` (v3 path)
2. **Operation type mapping** — `OperationType` enum (v1) vs `operationType.Method` string (v3 path)
3. **Namespace imports** — `Microsoft.OpenApi.Models` (v1) vs `Microsoft.OpenApi` (v3 path)
4. **Vendor extension writing** — `OpenApiString` (v1) vs `JsonNodeExtension` (v3 path)
5. **Vendor extension reading** — `OpenApiString` cast (v1) vs v3 API
...
- Remove `Microsoft.OpenApi.Readers` package (v1-only, superseded by v3 packages)
- Keep only the v3 code paths, simplified and inlined
...
Replace conditional ItemGroups with single unconditional references. Use Microsoft.OpenApi 3.x (latest). YAML reading is no longer built into the core package — add `Microsoft.OpenApi.YamlReader` 3.x as a separate dependency.
...
### D3: OpenApiSpecLoader — inline v3 parser path, delete LoadV1

Remove `LoadV1()` entirely. Inline the v3 parser path as the body of `Load()`. Remove the `#if` dispatch. The v3 path uses `OpenApiDocument.Parse(content, settings: settings)` which returns a `ReadResult` with `Document` and `Diagnostic` (singular).
...
### D5: SchemaTransformer — v3 vendor extension reading
...
In Microsoft.OpenApi v3, extensions are stored as `JsonNodeExtension`. The reading pattern becomes:
```csharp
...
Similarly for enum member values — `OpenApiString` casts become string extraction from the v3 enum representation.

For enum values in v3, `OpenApiSchema.Enum` contains `JsonNode` items. The extraction becomes:
```csharp
