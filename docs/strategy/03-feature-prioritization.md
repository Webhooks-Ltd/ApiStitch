...
**The MMVP answers one question**: If I show a .NET developer a NuGet package that takes an OpenAPI spec, produces clean Refit interfaces, maps schemas to their existing C# types, and runs on every build — do they care?

| # | Feature | Description | Rationale |
|---|---------|-------------|-----------|
| 1.1 | **Refit Interface Generation** | Generate Refit interface definitions from OpenAPI operation paths. One interface per tag (or configurable grouping). `[Get("/pets/{id}")]` with typed request/response. | Refit has 153.3M NuGet downloads — it is the dominant REST client pattern in .NET. Starting with Refit output maximizes the audience. The generated code is minimal (interfaces + attributes) which means less surface area for bugs and cleaner output. Refitter proves the demand; ApiStitch adds type reuse on top. |
...
| 1.10 | **CancellationToken on Every Method** | Every generated HTTP method accepts a `CancellationToken` parameter (with default `default`). | Table stakes for production .NET code. Refit interfaces support it natively. Typed clients must propagate it. This is a design constraint, not a feature to build — but it must be enforced from day one. Kiota is inconsistent about this. NSwag makes it optional via configuration. ApiStitch makes it non-negotiable. |

**MMVP scope boundary**: 10 features. One output style (Refit). Two type reuse mechanisms (YAML mapping + namespace exclusion). One spec version (OpenAPI 3.0). Core schemas only. No typed HttpClient wrappers. No extension method output. No CLI tool. No attribute-based type discovery. No schema-level overrides beyond skip. No pagination/IAsyncEnumerable. The MMVP is: parse a spec, map types, generate Refit interfaces + models, run on build.
...
| # | Feature | Description | Rationale |
|---|---------|-------------|-----------|
| 2.1 | **Typed HttpClient Wrappers** | Generate interface + concrete class pairs with `HttpClient` injection. Full IHttpClientFactory integration. The typed client pattern Microsoft recommends for production HTTP clients. | The second output style. Teams that don't use Refit (or can't, due to AOT constraints with Refit's Castle.DynamicProxy) need concrete typed clients. This is the Microsoft-documented pattern for IHttpClientFactory. Implementation reuses the same operation parsing as Refit output — only the code emitter differs. |
...
**MVP scope boundary**: 20 total features (10 MMVP + 10 MVP). Two output styles (Refit + typed HttpClient). Schema-level overrides. Tag/path filtering. Open enums. Partial classes. CLI tool. OpenAPI 3.1. The MVP is what you recommend to a team and say "this handles real-world specs."

**Estimated effort**: ~100-140 additional hours (5-7 weeks at 20hr/week). Typed HttpClient output (~25%) shares parsing logic with Refit but needs a new emitter. Schema overrides and filtering (~20%). OpenAPI 3.1 (~15%). CLI (~10%). Everything else is incremental (~30%).
...
| # | Feature | Description | Rationale |
|---|---------|-------------|-----------|
| 3.1 | **Extension Method Output Style** | Generate `public static async Task<Pet> GetPetAsync(this HttpClient client, int id, CancellationToken ct = default)` extension methods. The lightest-weight output: no interfaces, no DI registration, just methods on HttpClient. | The third output style. For internal service clients where Refit's magic or typed client ceremony is overkill. Extension methods are the most discoverable pattern — `httpClient.GetPetAsync()` appears in IntelliSense automatically. Deferred to v1 because Refit + typed client cover the vast majority of use cases, and three output backends triple the testing matrix. |
...
### Why Refit output first, not typed HttpClient?

Refit has 153.3M NuGet downloads. It is the most popular REST client pattern in .NET. Generating Refit interfaces produces the least code (interfaces + attributes, no implementation classes), which means the smallest surface area for bugs, the cleanest diff on version bumps, and the most inspectable output for a new user evaluating ApiStitch. Typed HttpClient wrappers follow in MVP because they serve a different audience (AOT, maximum control), but Refit is the widest net.
